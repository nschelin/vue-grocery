	...
	<div class="column form-container" v-if="dinner !== null">
		<form @submit.prevent="saveDinner()">
			...
			<div class="field is-horizontal">
				<div class="field-body">
					<div class="field">
						<h3 class="subtitle" v-if="dinner.products.length > 0">
							Products
						</h3>
					</div>
					<div class="field">
			...
			<div v-if="dinner.products.length > 0">
				<div class="box" style="margin-bottom: 20px;">
					<div
						...
						v-for="(product, index) in dinner.products"
						:key="index"
					>
						<div class="field-body">
							<div class="field">
							...
							<div class="field">
								<div class="control is-clearfix">
									<button
										...
										@click="
											dinner.products.splice(index, 1)
										"
									>
										Remove
									</button>
			...
			<div class="card">
				...
				<div class="card-content">
					<ProductDetails
						v-model="newProduct"
						:active="dialog"
						@productSaved="handleSavedProduct"
					/>
				</div>
			</div>
		</b-modal>
...
export default {
	...
	computed: {
		...mapState({
			products: state =>
				state.productsMod.products
					.slice()
					.sort((a, b) => (a.name > b.name ? 1 : -1))
		}),
		total: function() {
			return this.dinner !== null
				? this.dinner.products.reduce(
						(sum, product) =>
							(sum += product.price ? product.price : 0),
						0
				  )
				: 0;
		},
		noSelection: function() {
			...
			if (this.dinner.products.length === 0) return false;

			// check if any product hasn't been selected or not by check if any objects with _id of a product
			const index = this.dinner.products.findIndex(
				c => !c.hasOwnProperty('_id')
			);
	...
	mounted() {
		this.$nextTick(() =>
			setTimeout(() => this.$refs.dinnerName.focus(), 100)
		);
